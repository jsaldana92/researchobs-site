...
import snaImage from "../assets/features_sna_heatmaps/featues_sna.png";
...
const heatmapCells = [
  // cold/light activity, placed on the same invisible grid
  ...
  // central green/yellow activity around the green circle near A
...
export default function FeatureAnalysisSection() {
  const snaSectionRef = useRef(null);
  const heatmapSectionRef = useRef(null);
  const snaInView = useInView(snaSectionRef, { once: true, amount: 0.28 });
  const heatmapInView = useInView(heatmapSectionRef, {
    once: true,
    amount: 0.28,
  });
  ...
  return (
    <>
      <section
        ref={snaSectionRef}
        className="overflow-hidden bg-linear-to-br from-[#b7c8ba] via-[#e8ece5] to-[#f6f2e8] px-6 py-20 text-slate-900 sm:py-24"
      >
        <div className="mx-auto max-w-7xl">
          <div className="grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
            <div>
              ...
              <h2 className="mt-4 max-w-xl text-4xl font-black tracking-tight text-slate-900 sm:text-5xl">
                Evaluate group dynamics and social connections
              </h2>

              <p className="mt-5 max-w-xl text-base leading-7 text-slate-700 sm:text-lg">
                Convert observation records into social network visualizations
                so you can see who interacts with whom, compare connection
                strength, and identify patterns in group behavior.
              </p>
            </div>

            <motion.div
              initial={{ opacity: 0, y: 24, scale: 0.94 }}
              animate={snaInView ? { opacity: 1, y: 0, scale: 1 } : undefined}
              transition={{
                duration: 0.7,
                ease: [0.22, 1, 0.36, 1],
                delay: 0.12,
            ...
            >
              ...
              <img
                src={snaImage}
                alt="ResearchObs social network analysis visualization"
                className="relative z-10 w-full rounded-3xl object-contain shadow-[0_18px_45px_rgba(7,27,29,0.10)]"
                loading="lazy"
                decoding="async"
              />
            </motion.div>
          ...
          <div className="relative mx-auto mt-14 flex max-w-6xl flex-col items-center justify-center gap-6 lg:flex-row lg:gap-8">
            ...
            >
              <div className="aspect-square w-full overflow-hidden rounded-3xl bg-white">
                ...
                >
                  <Heatmap3DModel active={heatmapInView} />
                </Canvas>
              </div>
            </motion.figure>
          </div>
